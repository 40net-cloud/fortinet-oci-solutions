resource "oci_core_vcn" "fortisandbox" {
  count = local.create_new_vcn ? 1 : 0

  compartment_id = var.compartment_ocid
  cidr_blocks    = [var.vcn_cidr]
  display_name   = var.vcn_display_name
  dns_label      = "fortisandbox"
}
resource "oci_core_internet_gateway" "fortisandbox" {
  count = local.create_new_vcn ? 1 : 0
...
resource "oci_core_route_table" "frontend" {
  ...
  route_rules {
    ...
    network_entity_id = oci_core_internet_gateway.fortisandbox[0].id
  }
}
