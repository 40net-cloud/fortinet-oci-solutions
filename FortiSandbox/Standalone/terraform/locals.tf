locals {
  ...
  normalized_fortisandbox_version = trimspace(var.fortisandbox_version)

  fortisandbox_packages = flatten([
    for listing in local.listings : [
      ...
      if lower(trimspace(listing.name)) == "fortinet fortisandbox zero-day threat protection (byol)"
    ]
  ])

  matched_package = try(
    one([
      for pkg in local.fortisandbox_packages : pkg
      if pkg.license_type == local.normalized_license_type && pkg.version == local.normalized_fortisandbox_version
    ]),
    null
  )

  selected_vcn_id          = local.use_existing_vcn ? var.vcn_id : oci_core_vcn.fortisandbox[0].id
